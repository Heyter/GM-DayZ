...
function serverguard.PrintConsole(text)
    MsgC(serverguard.NotifyColors[SERVERGUARD.NOTIFY.SKY_BLUE], "[" .. os.date("%H:%M") .. "][ServerGuard] ", color_white, text)
end
...
serverguard.NotifyColors[SERVERGUARD.NOTIFY.SKY_BLUE] = Color(0, 255, 255)
...
if (SERVER) then
    ...
    --[[= #args]]
    local function Notify(args, lenargs)
