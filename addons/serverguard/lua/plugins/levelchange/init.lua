    ...
    for k, gm in ipairs(GamemodeList) do
        ...
        if (Maps and gm.maps ~= "") then
            for k, pattern in ipairs(Maps) do
                ...
                if not (plugin.MapPrefixes[Name]) then
                    plugin.MapPrefixes[Name] = {}
                end

                if not (table.HasValue(plugin.MapPrefixes[Name], patt_str)) then
        ...
        for i = 1, #plugin.MapPrefixes[map_cat] do
            for _, map in pairs(lc_maps) do
                ...
                if (map:lower():find("^" .. map_prfx[i]) and map_noext ~= plugin.CurrentMap) then
                    if not (all_maps[map_cat]) then
                        table.Merge(all_maps, {
                            [map_cat] = {}
                        })
                    end
