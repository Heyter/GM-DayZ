...
surface.CreateFont("sg.mapchange.divider", {
    font = "Roboto",
    size = 18,
    weight = 800
})

surface.CreateFont("sg.mapchange.curmap", {
    font = "Roboto",
    size = 16,
    weight = 800
})
...
function category:Create(base)
    ...
    function category.mapList:Think()
        if (CurTime() < category.UpdateMapsTimer and not category.LoadedMaps) then
            category.UpdateMapsTimer = CurTime() + ThinkTime
            category:Update()
        end
    end
...
function category:Update()
    if (self.ServerMaps and not self.LoadedMaps) then
    ...
    elseif (not self.RequestedMaps) then
