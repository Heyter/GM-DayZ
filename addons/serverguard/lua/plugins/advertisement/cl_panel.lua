...
surface.CreateFont("tiger.advertisement.background", {
    font = "Arial",
    size = 14,
    weight = 400,
    blursize = 2
...
function category:Create(base)
    ...
    function newAdvertisement:DoClick()
        NewAdvertisementWindow("Add an advertisement", "Create", function(text, interval, color)
            serverguard.netstream.Start("sgCreateAdvertisement", {string.sub(text, 1, 256), math.Clamp(math.Round(tonumber(interval)), 10, 86400), color})
...
serverguard.netstream.Hook("sgGetAdvertisement", function(data)
    ...
    function panel:OnMousePressed()
        ...
        menu:AddOption("Edit advertisement", function()
            EditAdvertisementWindow("Modify an advertisement", "Modify", {
                text = data.text,
                interval = data.interval,
                color = data.color
            }, function(text, interval, color)
                serverguard.netstream.Start("sgChangeAdvertisement", {self.id, string.sub(text, 1, 256), math.Clamp(tonumber(interval), 10, 86400), color})
