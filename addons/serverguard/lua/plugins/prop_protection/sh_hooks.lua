...
local function CANNOT_USE(player, owner)
    return (not serverguard.player:HasPermission(player, "Bypass Prop-Protection") and owner ~= CPPI.CPPI_DEFER) and (IsValid(owner) and (owner ~= player or not plugin.IsFriend(owner, player)))
end
...
function plugin.CanDrive(player, entity)
    ...
    if ((entity:CPPIGetOwner() == game.GetWorld() or entity:CPPIGetOwner() == CPPI.CPPI_DEFER) and not bBypassPropProtection) then return false end

    if (IsValid(entity)) then
        if (entity:IsPlayer() and not bBypassPropProtection) then return false end
        local owner = entity:CPPIGetOwner()
        if (CANNOT_USE(player, owner)) then return false end
    end
end
...
function plugin.CanProperty(player, property, entity)
    ...
    if (IsValid(entity)) then
        if (property == "bonemanipulate" and entity:IsPlayer() and not bBypassPropProtection) then return false end
        local owner = entity:CPPIGetOwner()
        if (CANNOT_USE(player, owner)) then return false end
    end
end
...
function plugin.CanTool(player, trace, toolMode)
    ...
    if ((entity:CPPIGetOwner() == game.GetWorld() or entity:CPPIGetOwner() == CPPI.CPPI_DEFER) and not bBypassPropProtection and entity ~= game.GetWorld()) then return false end
...
function plugin.PhysgunPickup(player, entity)
    ...
    if (serverguard.player:HasPermission(player, "Bypass Prop-Protection") and not entity:IsPlayer()) then return true end
    if ((entity:CPPIGetOwner() == game.GetWorld() or entity:CPPIGetOwner() == CPPI.CPPI_DEFER)) then return false end

    if (not entity:IsPlayer() and not IsValid(entity:CPPIGetOwner()) and (entity:CPPIGetOwner() ~= game.GetWorld() and entity:CPPIGetOwner() ~= CPPI.CPPI_DEFER)) then
        if (SERVER) then
    ...
    if (not entity:IsPlayer() and (not IsValid(entity:CPPIGetOwner()) or IsValid(entity:CPPIGetOwner()) and (entity:CPPIGetOwner() == player or plugin.IsFriend(entity:CPPIGetOwner(), player)))) then return true end
