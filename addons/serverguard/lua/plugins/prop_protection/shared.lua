...
function plugin.IsFriend(owner, target)
        ...
        if (IsValid(pPlayer)) then
            for i2 = 1, #buddies do
                ...
                if (pPlayer:SteamID() == steamID) then
                    if (not table.HasValue(friends, pPlayer)) then
    ...
    for i = 1, #friends do
        ...
        if (not IsValid(friend)) then
            table.remove(friends, i)
        else
            if (not table.HasValue(buddies, friend:SteamID())) then
                table.remove(friends, i)
            end
        end
    end
    ...
    for k, friend in pairs(friends) do
        if (friend == target) then return true end
    end
end