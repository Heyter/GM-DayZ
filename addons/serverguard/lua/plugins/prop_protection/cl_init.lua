...
hook.Add("serverguard.LoadPlayerData", "prop_protection.LoadPlayerData", function(player)
    g_serverGuard.prop_protection = g_serverGuard.prop_protection or {
        friends = {},
        buddies = {}
    }
end)
    ...
    if (lastPropCheck < CurTime()) then
        ...
        if (IsValid(trace.Entity)) then
            if (trace.Entity.serverguard) then
                if (not trace.Entity.serverguard.updated) then
...
plugin:Hook("HUDPaint", "prop_protection.HUDPaint", function()
    if (not hook.Call("serverguard.HideOwner")) then
        local trace = LocalPlayer():GetEyeTrace()
        ...
        if (IsValid(trace.Entity) and not trace.Entity:IsPlayer()) then
            if (trace.Entity.serverguard) then
                ...
                if (not IsValid(trace.Entity.serverguard.owner) and trace.Entity.serverguard.owner ~= game.GetWorld()) then
...
serverguard.netstream.Hook("sgPropProtectionClearEntityData", function(entity)
