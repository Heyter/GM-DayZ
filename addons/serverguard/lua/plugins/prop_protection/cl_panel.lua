...
local function CreateBuddiesPanel(base)
    ...
    function buddiesPanel.list:Think()
        ...
        for i = 1, #players do
            ...
            if (not IsValid(pPlayer.propProtection) and pPlayer ~= LocalPlayer()) then
...
function category:Create(categoryBase)
    ...
    if (not managePermission and not blacklistPermission) then
            ...
            if (not panelBase.created) then
                ...
                category.blacklistLayout.Think = function(_panel)
                    if (category.blacklist and #category.blacklist.queue > 0 and CurTime() >= category.nextBlacklistQueueTime) then
                        ...
                        -- load more panels at a time if there are a lot of props
                        if (#category.blacklist.queue > 1000) then
                ...
                add.DoClick = function(_panel)
                    util.CreateStringRequest("Add prop to blacklist", function(text)
                        if (text ~= "") then
                            category:AddBlacklistProp(text, true)
                        end
                    end, "Add", function(text) end, "Cancel")
                end
                ...
                remove.DoClick = function(_panel)
                    util.CreateStringRequest("Remove prop from blacklist", function(text)
                        if (text ~= "") then
...
function category:AddBlacklistProp(model, bSend)
    if (not table.HasValue(self.blacklist.list, model)) then
        table.insert(self.blacklist.list, model)
        table.insert(self.blacklist.queue, model)
    end

    -- we send anyway server will handle notifies
    if (bSend) then
...
-- HACK
plugin:Hook("PopulatePropMenu", "prop_protection.PopulatePropMenu", function()
