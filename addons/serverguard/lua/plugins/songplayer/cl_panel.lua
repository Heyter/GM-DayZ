...
function PANEL:Think()
    ...
    if (gui.MouseX() > x and gui.MouseX() < x + width and gui.MouseY() > y and gui.MouseY() < y + height) then
        if (not self:GetAnimating() and self.currentAlpha < 255) then
        ...
        if (not self:GetAnimating() and CurTime() > self.lastInteractTime + 6) then
