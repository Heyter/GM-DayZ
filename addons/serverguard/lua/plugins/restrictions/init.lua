...
local function ReachedLimit(player, limitType, message)
    ...
    if (amount >= cvars.Number("sbox_max" .. string.lower(limitType))) then return false, false end
...
plugin:Hook("PlayerSpawnEffect", "restrictions.PlayerSpawnEffect", function(player, model)
    ...
    if (reachedLimit) then return false end
    if (not reachedLimit and bForce) then return true end
end)
