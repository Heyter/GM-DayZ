...
plugin:Hook("serverguard.RanCommand", "logs.RanCommand", function(player, commandTable, bSilent, arguments)
    if (util.IsConsole(player)) then
        plugin:Log(string.format("Console ran command \"%s %s\"", commandTable.command, table.concat(arguments, " ")))
    ...
    if (arguments and table.concat(arguments, " ") ~= "") then
        plugin:Log(string.format("<%s> %s ran command \"%s %s\"", steamID, playerNick, commandTable.command, table.concat(arguments, " ")))
    else
        plugin:Log(string.format("<%s> %s ran command \"%s\"", steamID, playerNick, commandTable.command))
    end
end)
...
local function SendLogChunk(player, logFolder, logFile)
    if (player.sgLogChunks and player.sgLogChunks[logFolder] and player.sgLogChunks[logFolder][logFile]) then
        if (#player.sgLogChunks[logFolder][logFile] >= 1) then
            ...
            serverguard.netstream.Start(player, "sgSendLogChunk", {logFolder, logFile, chunk})

            table.remove(player.sgLogChunks[logFolder][logFile], 1)
        else
            serverguard.netstream.Start(player, "sgSendLogChunk", {logFolder, logFile, true})
        end
    end
end
...
serverguard.netstream.Hook("sgRequestLogData", function(player, data)
    if (player:IsAdmin()) then
        ...
        for k, v in ipairs(files) do
            ...
            if (not player.sgLogChunks) then
                player.sgLogChunks = {}
            end

            if (not player.sgLogChunks[dateFormat]) then
...
serverguard.netstream.Hook("sgRequestLogFolders", function(player, data)
    if (player:IsAdmin()) then
        ...
        for k, v in ipairs(folders) do
            ...
            serverguard.netstream.Start(player, "sgSendLogFolders", {v, #files})
        end
    end
end)
