...
local function explode(separator, str, limit)
    ...
    while true do
        ...
        if newpos ~= nil then
            ...
            if limit and #t > limit then return t end -- Reached limit
...
local function parseKeyValues(str, convert)
    ...
    if convert and table.Count(current_table) == 1 and type(current_table.Out) == "table" then
...
function plugin.ulx:bans()
    ...
    if (not bans) then return false, "no ban data found" end
