...
function command:Execute(player, silent, arguments)
    ...
    if (not plugin:CreateVote(title, arguments, nil, true)) then
...
function command:Execute(player, silent, arguments)
    if (not plugin:CreateVote("Vote for the next map", arguments, function(option)
...
function command:Execute(player, silent, arguments)
    ...
    if (IsValid(target) and util.PlayerMatchesImmunity(SERVERGUARD.IMMUNITY.LESS, player, target)) then
        if (not plugin:CreateVote("Kick " .. target:Name() .. "?", {}, function(option)
            if (option == "Yes") then
...
function command:Execute(player, silent, arguments)
    ...
    if (IsValid(target) and util.PlayerMatchesImmunity(SERVERGUARD.IMMUNITY.LESS, player, target)) then
        if (not plugin:CreateVote("Ban " .. target:Name() .. " for 30 minutes?", {}, function(option)
            if (option == "Yes") then
