...
serverguard.pollinterval = CreateConVar("serverguard_pollinterval", "0", {FCVAR_PROTECTED, FCVAR_SERVER_CAN_EXECUTE}, "How often local ban data is synced with the database (in seconds). Set to 0 to disable.")
...
function serverguard:playerSend(from, to, force)
    ...
    for i = 1, 4 do
        ...
        if i == 4 and tr.Hit and not force then return false end
    end
...
hook.Add("PhysgunPickup", "serverguard.jail.PhysgunPickup", function(player, entity)
    if (IsValid(entity) and entity.sg_jail or player.sg_jail) then return false end
end)
...
hook.Add("EntityTakeDamage", "serverguard.jail.EntityTakeDamage", function(target, dmgInfo)
    ...
    if (IsValid(attacker) and attacker.sg_jail) then return true end
end)

local function JailBlock(player)
    if (IsValid(player) and player.sg_jail) then return false end
end
...
local function RestrictBlock(player)
    if (IsValid(player)) then
        ...
        if (restrictTime and restrictTime > os.time()) then
            if (not player.nextRestrictNotify or CurTime() > player.nextRestrictNotify) then
...
local function LoadBanData()
    timer.Simple(2, function()
        if (hook.Call("serverguard.LoadBanData", nil)) then return end
...
local function CheckPlayerBanned(communityID, ip, serverPassword, enteredPassword, name)
    if (hook.Call("serverguard.ShouldCheckPlayerBanned", nil, communityID, ip, serverPassword, enteredPassword, name) == false) then return end
    ...
    if (data) then
        ...
        if (endTime > 0 and endTime <= os.time()) then
            ...
            if (endTime <= 0) then return false, string.format("You are permanently banned.\n\nAdmin: %s\nReason: %s", data.admin, data.reason) end
...
function serverguard:BanPlayer(admin, player, length, reason, bKick, bSilent, adminNameOverride)
    ...
    if (type(player) == "Player") then
        if (not hook.Call("serverguard.PlayerBanned", nil, player, length, reason, admin)) then
            if (not bIsConsole) then
            ...
            BAN_INSERT(player:SteamID(), player:SteamID64(), player:Name(), reason, startTime, endTime, adminName, (player:IPAddress():gsub(":%d+", "")))
        ...
        if (IsValid(playerObj)) then
            ...
            if (string.find(player, "STEAM_(%d+):(%d+):(%d+)")) then
                local callback = function(result, status, lastID)
                    ...
                    if (not hook.Call("serverguard.PlayerBannedBySteamID", nil, player, length, reason, admin, name)) then
                        if (not bIsConsole) then
                        ...
                        BAN_INSERT(player, util.SteamIDTo64(player), name, reason, startTime, endTime, adminName, "")
...
function serverguard:UnbanPlayer(steamID, admin)
    if (hook.Call("serverguard.PlayerUnbanned", nil, steamID, admin)) then return end
...
local function serverGuard_QueryBans(player)
    if (not player.nextBanQuery) then
        ...
        if istable(ULib) and type(ULib.parseKeyValues) == "function" and type(ULib.removeCommentHeader) == "function" and type(ULib.fileRead) == "function" then
            ...
            for k, v in pairs(ulxBans) do
                ...
                if (unbanTime == 0 or unbanTime > os.time()) then
                    BAN_INSERT(v.steamID, util.SteamIDTo64(v.steamID), v.name or v.steamID, v.reason, v.time or 0, v.unban or 0, v.admin or "", "")
                    c = c + 1
                end
            end
