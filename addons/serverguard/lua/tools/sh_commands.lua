...
--
serverguard.kickReasons = {"Spammer", "Crashed server", "Mingebag", "Griefer", "Foul language", "Disobeying the rules", "DDoS Threats"}
...
function command:ContextMenu(player, menu, rankData)
    ...
    for _, v in pairs(serverguard.banLengths) do
...
function command:Execute(player, silent, arguments)
    ...
    elseif (not util.IsConsole(player)) then
            ...
            elseif (string.SteamID(arguments[1])) then
                ...
                queryObj:Callback(function(result, status, lastID)
                    if (istable(result) and #result > 0) then
                        ...
                        if (not util.IsConsole(player) and target.steam_id == player:SteamID()) then
                            serverguard.Notify(player, SGPF("command_rank_cannot_set_own"))
                        ...
                        if (not playerRankData or serverguard.player:HasBetterImmunity(player, playerRankData.immunity)) then
...
function command:Execute(player, silent, arguments)
    ...
    if (pluginTable) then
        ...
        if (pluginTable.gamemodes and #pluginTable.gamemodes > 0) then
            if (not table.HasValue(pluginTable.gamemodes, engine.ActiveGamemode())) then
                bWhitelisted = false
            end
        end

        serverguard.netstream.Start(nil, "sgGetPluginStatus", {uniqueID, not pluginTable.toggled})

        serverguard.plugin:Toggle(uniqueID, not pluginTable.toggled)

        if not bWhitelisted then
...
function command:Execute(player, silent, arguments)
    ...
    if (IsValid(target)) then
        if (not serverguard.player:HasBetterImmunity(player, serverguard.player:GetImmunity(target))) then
            serverguard.Notify(player, SERVERGUARD.NOTIFY.RED, "This player has a higher immunity than you.")
...
function command:Execute(pPlayer, silent, arguments)
    ...
    if (util.IsConsole(pPlayer)) then return end
...
function command:ContextMenu(pPlayer, menu, rankData)
    local option = menu:AddOption("Toggle God Mode", function()
        if (not pPlayer:HasGodMode()) then
...
-- The slap command.
--
local slapSounds = {Sound("physics/body/body_medium_impact_hard1.wav"), Sound("physics/body/body_medium_impact_hard2.wav"), Sound("physics/body/body_medium_impact_hard3.wav"), Sound("physics/body/body_medium_impact_hard5.wav"), Sound("physics/body/body_medium_impact_hard6.wav"), Sound("physics/body/body_medium_impact_soft5.wav"), Sound("physics/body/body_medium_impact_soft6.wav"), Sound("physics/body/body_medium_impact_soft7.wav")}
...
function command:OnPlayerExecute(_, target, arguments)
    ...
    target:SetNoTarget(not target.sg_notarget)
    target.sg_notarget = not target.sg_notarget
...
if (SERVER) then
    hook.Add("PlayerCanHearPlayersVoice", "serverguard.PlayerCanHearPlayersVoice", function(listener, speaker)
        if (speaker.sgGagged) then return false end
    end)
end
...
function command:CloakHooks()
    ...
    for k, user in ipairs(player.GetAll()) do
...
local function FreezeBlock(player)
    if (player.sg_isFrozen) then return false end
end
...
FreezeHookHelper("PlayerSpawnEffect", "PlayerSpawnNPC", "PlayerSpawnObject", "PlayerSpawnProp", "PlayerSpawnRagdoll", "PlayerSpawnSENT", "PlayerSpawnSWEP", "PlayerSpawnVehicle")
...
hook.Add("PlayerNoClip", "serverguard.PlayerNoClip", function(player)
    if (player.sg_jail or player:GetNetworkedBool("serverguard_jailed", false)) then return false end
    if (serverguard.player:HasPermission(player, "Noclip")) then return true end
end)
...
function command:Execute(player, silent, arguments)
    if (not player.sg_incognito) then
...
local function friendsChatAddTable(tTbl, cCol)
    ...
    local cWhite, cRed = serverguard.NotifyColors[SERVERGUARD.NOTIFY.RED], serverguard.NotifyColors[SERVERGUARD.NOTIFY.WHITE]
    local tChat = {}
...
serverguard.netstream.Hook("sgGetFriends", function(tData)
    ...
    if table.Count(tFriends) == 0 then
        chat.AddText(serverguard.NotifyColors[SERVERGUARD.NOTIFY.RED], "No friends were found for your target!")

        return
    end

    local cGreen, cWhite, cRed = serverguard.NotifyColors[SERVERGUARD.NOTIFY.GREEN], serverguard.NotifyColors[SERVERGUARD.NOTIFY.WHITE], serverguard.NotifyColors[SERVERGUARD.NOTIFY.RED]
    chat.AddText(cWhite, pTargetsName .. "'s Friends")
    chat.AddText(cWhite, "Friends: ", friendsChatAddTable(tFriends, cRed))
    chat.AddText(cWhite, "Mutal Friends: ", friendsChatAddTable(tMutualFriends, cGreen))
end)
...
hook.Add("ShouldCollide", "serverguard.commands.nocollide", function(e1, e2)
    if (e1:IsPlayer() and e1.sg_NoCollide or e2:IsPlayer() and e2.sg_NoCollide) then return false end
end)
...
function command:OnPlayerExecute(_, target)
    ...
    serverguard.netstream.Start(player.GetAll(), "sgSetCollision", {target, target.sg_NoCollide})
