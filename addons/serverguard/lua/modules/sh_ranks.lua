...
if (SERVER) then
    ...
    hook.Add("CAMI.PlayerHasAccess", "serverguard.ranks.CAMI.PlayerHasAccess", function(actor, privilege, callback, target, extra)
        -- it's our permission
        if (serverguard.permission:GetAll()[privilege]) then
    ...
    hook.Add("serverguard.mysql.CreateTables", "serverguard.ranks.CreateTables", function()
        local query = serverguard.mysql:Create("serverguard_ranks")
			...
			query:Create("color", "VARCHAR(41) NOT NULL")
			query:Create("immunity", "INT(11) NOT NULL")
			query:Create("targetable", "INT(11) NOT NULL DEFAULT '0'")
			query:Create("banlimit", "INT(11) NOT NULL DEFAULT '0'")
			query:Create("texture", "VARCHAR(255) NOT NULL")
    ...
    function serverguard.ranks:RemoveRank(uniqueID, bSaveless)
        if (serverguard.ranks.stored[uniqueID]) then
            if (serverguard.ranks:IsDefault(uniqueID)) then return false, "Default ranks cannot be removed!" end
    ...
    function serverguard.ranks:GetVariable(lookup, variable)
        ...
        if (type(lookup) == "Player" and IsValid(lookup) and lookup:IsPlayer()) then
            rankData = serverguard.ranks:GetRank(serverguard.player:GetRank(lookup))
        ...
        if (serverguard.player:HasPermission(player, "Edit Ranks")) then
            ...
            if (field == "Permissions") then
                if (uniqueID ~= "founder") then
                    ...
                    serverguard.netstream.Start(nil, "sgNetworkRankData", {uniqueID, field, value})
                end
            else
                serverguard.ranks:SetData(uniqueID, field, value)
        ...
        queryObj:Callback(function(result, status, lastID)
            if (istable(result) and #result > 0) then
                for k, v in pairs(result) do
                    ...
                    serverguard.netstream.Start(player, "sgGetRankList", {v.steam_id, v.rank, v.name, expires_in, os.date("*t", os.difftime(os.time(), v.last_played)) or "Unknown"})
                end
            end
        end)
    ...
    serverguard.netstream.Hook("sgNetworkRankInfo", function(data)
        ...
        elseif (type == SERVERGUARD.NETWORK.DECODE) then
            -- The only thing using this are the flags.
            value = tostring(data.value)
            serverguard.flags:ApplyFlags(serverguard.ranks.stored[unique], string.Explode(",", value))
        elseif (type == SERVERGUARD.NETWORK.COLOR) then
            -- Only color using this.
            value = Color(value.r, value.g, value.b, 255)
        elseif (type == SERVERGUARD.NETWORK.CUSTOM) then
            -- Restrictions.
...
function serverguard.ranks:FindByID(uniqueID)
    if (self.stored[string.lower(uniqueID)]) then return self.stored[string.lower(uniqueID)] end
end
...
function serverguard.ranks:IsDefault(identifier)
    if (type(identifier) == "string") then return self.default[string.lower(identifier)] end
end
...
function serverguard.ranks:HasPermission(rank, identifier)
    if (rank == "founder") then return true end
