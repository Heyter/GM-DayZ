...
function serverguard.phrase:Get(unique, ...)
    if (not languages[self.currentLanguage][unique]) then return "" end
    local phrase = languages[self.currentLanguage][unique]
    local result = ""

    if (istable(phrase)) then
        for _, v in pairs(phrase) do
            if (isstring(v)) then
                result = result .. v
            end
        end
...
function serverguard.phrase:GetFormatted(unique, ...)
    if (not languages[self.currentLanguage][unique]) then
