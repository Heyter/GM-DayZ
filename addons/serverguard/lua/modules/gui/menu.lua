...
local hints = {"Try binding '+serverguard_menu' to open the menu, or '+serverguard_quickmenu' for quick access to commands.", "You can search for categories on the left simply by typing the name of the category.", "When using commands, you can specify *, ^, or #admin as the name to target everyone, yourself, or only admins.", "You can run any command from the console by typing 'sg <command> <arguments>'.", "You can return to your last position after using a teleport command by using the !return command.", "Clicking anywhere outside of the menu will also close it."}
...
function serverguard.menu.Close(bForce)
    if (IsValid(panelObject) and not panelObjectStay or bForce) then
...
local function CreateMenu(player)
    if (not IsValid(panelObject)) then
        panelObject = vgui.Create("tiger.base")
        panelObject:SetSize((ScrW() >= 950 and 950) or ScrW(), (ScrH() >= 650 and 650) or ScrH())
        panelObject:Rebuild()
        panelObject:Center()
    ...
    if (serverguard.player:HasPermission(pPlayer, "Quick Menu")) then
        ...
        for k, v in ipairs(player.GetSortedPlayers()) do
            ...
            for k2, v2 in util.SortedPairsByMemberValue(commandsTable, "command") do
                if (v2.ContextMenu and (not v2.permissions or serverguard.player:HasPermission(pPlayer, v2.permissions))) then
                    v2:ContextMenu(v, playerMenu, rankData)
                    bNoAccess = false
                end
            end
        end
