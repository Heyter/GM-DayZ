...
function category:Think(base)
    if (not category.loading.inProgress or CurTime() < category.loading.nextProcessTime) then return end
    ...
    for i = category.loading.currentIndex, math.min(category.loading.maxIndex, category.loading.currentIndex + 4) do
        ...
        if (not category.loading.banList[i]) then continue end
