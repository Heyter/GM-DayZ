...
function category:Create(base)
    ...
    function base.panel.list:Think()
        ...
        for i = 1, #players do
            ...
            if (not IsValid(pPlayer.managePanel)) then
                local rankData = serverguard.ranks:FindByID(serverguard.player:GetRank(pPlayer))
                ...
                function panel:OnMousePressed(code)
                    ...
                    for unique, data in pairs(commands) do
                        if (data.ContextMenu and (not data.permissions or serverguard.player:HasPermission(LocalPlayer(), data.permissions))) then
                            data:ContextMenu(self.player, menu, rankData)
                            bNoAccess = false
                        end
                    end
