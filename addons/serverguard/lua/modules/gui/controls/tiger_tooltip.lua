...
surface.CreateFont("tiger.tooltip", {
    font = "Arial",
    size = 14,
    weight = 400
})
...
function panel:Think()
    ...
    if (not self.parentPanel or not IsValid(self.parentPanel) and not self.collect) then
        self.collect = true
...
function registry.Panel:SetToolTipSG(text)
    ...
    if (not IsValid(self.tooltip_sg)) then
    ...
    function self:OnCursorExited()
        if (IsValid(self.tooltip_sg)) then
            self.tooltip_sg:AlphaTo(0, 0.2, 0, function()
                if (not self.sgToolTip and self.tooltip_sg and IsValid(self)) then
                    self.tooltip_sg:SetVisible(false)
                end
            end)
        end
