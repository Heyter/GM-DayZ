...
surface.CreateFont("tiger.list.text", {
    font = "Arial",
    size = 14,
    weight = 400
})

surface.CreateFont("tiger.list.column", {
    font = "Istok",
    size = 14,
    weight = 800
})
...
function panel:AddColumn(name, width)
    ...
    function column:Paint(w, h)
        ...
        if (base.sortColumn == self.sortID and not self.disabled) then
...
function panel:AddItem(...)
    ...
    function panel:Paint(w, h)
        ...
        if (self.last and not scrollbar:IsVisible()) then
            surface.DrawLine(0, h - 1, w, h - 1)
        end
    end
