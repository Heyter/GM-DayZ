...
function panel:PopulateArticle(title, date, stub, content)
    ...
    -- Colours.
    formatting = string.gsub(formatting, "{{tiger_news_bg}}", util.ColorString(backgroundColor, true))
    formatting = string.gsub(formatting, "{{tiger_news_title}}", util.ColorString(titleColor, true))
    formatting = string.gsub(formatting, "{{tiger_news_content}}", util.ColorString(contentColor, true))
    -- Content.
    formatting = string.gsub(formatting, "{{title}}", title)
    formatting = string.gsub(formatting, "{{date}}", date)
    formatting = string.gsub(formatting, "{{stub}}", stub)
    formatting = string.gsub(formatting, "{{content}}", content)
    self:SetHTML(formatting)
end

function panel:RepopulateArticle()
    if (not self:IsPopulated()) then return end
