...
function serverguard.plugin:Toggle(uniqueID, bToggled)
    if (not stored[uniqueID]) then return end
...
if (SERVER) then
    ...
    hook.Add("serverguard.LoadPlayerData", "plugin.LoadPlayerData", function(player)
        ...
        for uniqueID, data in pairs(plugins) do
            serverguard.netstream.Start(player, "sgGetPluginStatus", {uniqueID, data.toggled})
        end
    end)
    ...
    hook.Add("serverguard.PostLoadConfig", "serverguard.plugin.PostLoadConfig", function()
        for unique, v in pairs(stored) do
            if (stored[unique]) then
                ...
                if (stored[unique].gamemodes and #stored[unique].gamemodes > 0 and not v.forced) then
                    if (not table.HasValue(stored[unique].gamemodes, engine.ActiveGamemode())) then
                        bToggled = false
    ...
    function category:Create(base)
        ...
        for unique, data in pairs(plugins) do
            ...
            if (data.gamemodes and #data.gamemodes > 0) then
                if (not table.HasValue(data.gamemodes, engine.ActiveGamemode())) then
                    toggleButton.whitelist = false
                end
            end

            if (not toggleButton.whitelist) then
            ...
            function toggleButton:DoClick()
                if (serverguard.player:HasPermission(LocalPlayer(), "Manage Plugins")) then
                    if (not self.whitelist and not serverguard.plugin.Get(self.unique).toggled) then
                        local q = util.CreateDialog("Incompatible gamemode", "This gamemode is incompatible with this plugin. Load it anyways?", function()
