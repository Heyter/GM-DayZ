...
--[[= false]]
function player.GetSortedPlayers(bReverse)
...
function util.SortedPairsByMemberValue(tab, valueName, descending)
    ...
    table.SortByMember(sortedTable, valueName, not descending)
...
function util.PlayerMatchesIdentifier(player, identifier)
    ...
    return (IsValid(player) and identifier and result or string.lower(identifier) == string.lower(player:Name()) or string.lower(identifier) == string.lower(player:Nick()) or player:SteamID() == identifier or player:UniqueID() == identifier or player:SteamID64() == identifier or (player:IPAddress():gsub(":%d+", "")) == identifier)
end
...
function util.PlayerContainsIdentifier(player, identifier)
    ...
    return (IsValid(player) and identifier and result or (string.find(string.lower(player:Name()), string.lower(identifier), 0, true) or string.find(string.lower(player:Nick()), string.lower(identifier), 0, true) or player:SteamID() == identifier or player:UniqueID() == identifier or player:SteamID64() == identifier or (player:IPAddress():gsub(":%d+", "")) == identifier))
end
...
function util.PlayerMatchesImmunity(requiredImmunity, pPlayer, target, bNoTargetSelf)
    ...
    return result or (requiredImmunity == SERVERGUARD.IMMUNITY.EQUAL and serverguard.player:GetImmunity(target) == serverguard.player:GetImmunity(pPlayer)) or (requiredImmunity == SERVERGUARD.IMMUNITY.LESS and serverguard.player:GetImmunity(target) < serverguard.player:GetImmunity(pPlayer)) or (requiredImmunity == SERVERGUARD.IMMUNITY.LESSOREQUAL and serverguard.player:GetImmunity(target) <= serverguard.player:GetImmunity(pPlayer)) or (requiredImmunity == SERVERGUARD.IMMUNITY.ANY) or (not bNoTargetSelf and pPlayer == target)
end

local identifierTags = {
    -- Immunity is automatically checked beforehand so we don't have to do it here.
    ["^"] = {
        filter = function(callingPlayer, targetPlayer) return callingPlayer == targetPlayer end,
        negated = function(callingPlayer, targetPlayer) return callingPlayer ~= targetPlayer end
    },
    ["\\*"] = {
        filter = function(callingPlayer, targetPlayer) return true end,
        negated = function(callingPlayer, targetPlayer) return false end
    },
    ["#*"] = {
        filter = function(callingPlayer, targetPlayer, argument) return serverguard.player:GetRank(targetPlayer) == argument end,
        negated = function(callingPlayer, targetPlayer, argument) return serverguard.player:GetRank(targetPlayer) ~= argument end
    },
    ["@"] = {
        filter = function(callingPlayer, targetPlayer, argument) return callingPlayer:GetEyeTraceNoCursor().Entity == targetPlayer end,
        negated = function(callingPlayer, targetPlayer, argument) return callingPlayer:GetEyeTraceNoCursor().Entity ~= targetPlayer end
    }
}
...
function util.ExecuteOnPlayers(identifier, callingPlayer, requiredImmunity, func)
    if (not identifier or not callingPlayer or not requiredImmunity or not func) then return end
...
function util.GetListenServerHost()
    for k, v in ipairs(player.GetAll()) do
        if (v:IsListenServerHost()) then return v end
    end
...
function util.FindPlayer(identifier)
	...
	if (identifier:len() == 17) and (identifier:sub(1, 4) == '7656') then
    ...
    for _, v in ipairs(player.GetAll()) do
        ...
        if (v:UniqueID() == identifier or (v:IPAddress():gsub(":%d+", "")) == identifier or playerNick == identifier or playerName == identifier) then return v end

        if (string.find(playerNick, identifier, 0, true) or string.find(playerName, identifier, 0, true)) then
			return v
        end
    end
end
...
function string.Ownership(text, bSuffixOnly)
    ...
    if (bSuffixOnly) then return suffix end
...
function util.IsConsole(object)
    if (isentity(object) and not IsValid(object) and (object.EntIndex and object:EntIndex() == 0)) then return true end
...
function util.IsoDurationToSeconds(iso)
    local duration = 0
    local number = ""
    if (string.sub(iso, 1, 1) ~= "P") then return -1 end

    for i = 1, string.len(iso) do
        local character = string.sub(iso, i, i)
        if (character == "P" or character == "T") then continue end

        if (tonumber(character)) then
            number = number .. character
        end

        if (not tonumber(number)) then return -1 end
...
function util.ParseDuration(input)
    if (tonumber(input)) then
        ...
        if (number <= 0) then return 0, "Indefinitely", false end
    ...
    for i = 1, string.len(input) do
        ...
        if (not durationUnits[character]) then
            number = ""
            continue
        end

        if (tonumber(number) < 0 or tonumber(number) > 99) then
...
elseif (CLIENT) then
    ...
    function util.ColorLimit(color, r, g, b, a)
        return Color(math.min(color.r, r), math.min(color.g, g), math.min(color.b, b), math.min(color.a, a))
    end
    ...
    function util.ColorString(color, bExcludeAlpha)
        return color.r .. "," .. color.g .. "," .. color.b .. (not bExcludeAlpha and "," .. color.a or "")
    end
    ...
    function util.CreateStringRequest(title, ...)
        ...
        for k, v in ipairs(arguments) do
            if (isfunction(v)) then
                ...
                if (not arg or not isstring(arg)) then continue end
