...
function serverguard.command:FindByID(uniqueID)
    ...
    if (command) then
        ...
        for k, v in pairs(self.stored) do
            if (v.aliases and table.HasValue(v.aliases, uniqueID)) then return v end
        end
    end
end
...
if (SERVER) then
    ...
    local function RUN_COMMAND(commandTable, player, bIsSilent, arguments)
        ...
        if (commandTable.Execute) then
            ...
            if (not commandTable.bNoLog) then
                serverguard.PrintConsole(string.format("%s ran command \"%s %s\"\n", serverguard.player:GetName(player), commandTable.command, table.concat(arguments, " ")))
            end
            ...
            if (commandTable.bSingleTarget) then
                ...
                if (target and IsValid(target)) then
                    if (not util.PlayerMatchesImmunity(commandTable.immunity, player, target)) then
                ...
                targets = util.ExecuteOnPlayers(arguments[1], player, commandTable.immunity or SERVERGUARD.IMMUNITY.LESSOREQUAL, function(target) return commandTable:OnPlayerExecute(player, target, arguments) end)
            end

            if (not bIsSilent and commandTable.OnNotify and targets and #targets > 0 and not hook.Call("serverguard.CommandNotify", nil, player, targets, arguments)) then
        ...
        if (commandTable) then
            ...
            if (hook.Call("serverguard.PlayerCanUseCommand", nil, player, commandTable, bIsSilent, arguments) ~= false) then
                ...
                if (commandTable.permissions and #commandTable.permissions > 0) then
                    if (not serverguard.player:HasPermission(player, commandTable.permissions)) then
                ...
                if (not commandTable.arguments or #arguments >= #commandTable.arguments) then
                    RUN_COMMAND(commandTable, player, bIsSilent, arguments)
                elseif (not util.IsConsole(player) and #arguments == 0 and commandTable.arguments and #commandTable.arguments == 1 and commandTable.arguments[1] == "player") then
    ...
    hook.Add("PlayerSay", "serverguard.command.PlayerSay", function(pPlayer, text, m_bToAll, m_bDead)
        ...
        if (prefix == "!" or prefix == "~") then
            ...
            if (pPlayer.sgMuted and commandName ~= "unmute") then return "" end
            local commandExists = serverguard.command.Run(pPlayer, commandName, bSilent, unpack(arguments))
            if (commandExists) then return "" end
...
elseif (CLIENT) then
    ...
    timer.Create("serverguard.autocomplete.timer", FrameTime() * 8, 0, function()
