    ...
    if (isstring(player) and string.SteamID(player)) then
        ...
        queryObj:Callback(function(result, status, lastID)
            if (istable(result) and #result > 0) then
                ...
                insertObj:Insert("data", serverguard.von.serialize({
...
function serverguard.player:GetBanLimit(player)
    if (IsValid(player)) then return player:GetNetworkedInt("serverguard_banlimit", 0) end
end
...
function serverguard.player:SetupSpectate(player, mode)
    if (not player.sg_spectateData) then
    ...
    if (not player.spectatorMode) then
...
function serverguard.player:ChangeSpectatorMode(player, decrease)
    local spectatorModes = {OBS_MODE_ROAMING, OBS_MODE_CHASE, OBS_MODE_IN_EYE}
...
function serverguard.player:HasPermission(player, identifier)
    if (self:GetRank(player, true) == "founder") then return true end
    local permissionTable = serverguard.ranks:GetData(self:GetRank(player, true), "Permissions")

    if (permissionTable) then
        local identype = type(identifier)
        if (identype == "string") then return permissionTable[identifier] end

        if (identype == "table") then
            for k, v in pairs(identifier) do
                if (isstring(v) and permissionTable[v]) then return true end
            end
        end
    end
...
do
    ...
    function playerMeta:IsAdmin()
        if (self:IsSuperAdmin() or serverguard.player:HasPermission(self, "Admin")) then return true end
    ...
    function playerMeta:IsSuperAdmin()
        if (serverguard.player:HasPermission(self, "Superadmin")) then return true end
    ...
    function playerMeta:IsUserGroup(name)
        if (serverguard.player:GetRank(self) == name) then return true end
