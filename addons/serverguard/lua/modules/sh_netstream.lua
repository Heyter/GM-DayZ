...
local stored = {}
local chunks = {}
local bb_write, bb_read = include"bitbuf_glua/write.lua", include"bitbuf_glua/read.lua"
local coded = include"bitbuf_glua/table.lua"
...
serverguard.tables = {
    ...
    read = function() return decode(net.ReadData(net.ReadUInt(32))) end
}
...
if (SERVER) then
    ...
    function serverguard.netstream.StartChunked(pPlayer, name, data)
        ...
        if (recipients) then
            local splitData = serverguard.netstream.Split({
                data = (data or 0)
            })
    ...
    net.Receive("ServerGuardChunkedDS", function(length, player)
        ...
        if (not chunkQueue[NS_DS_NAME] or not chunkQueue[NS_DS_NAME].data[NS_DS_NEXT] or not table.HasValue(chunkQueue[NS_DS_NAME].recipients, player)) then return end
    ...
    net.Receive("ServerGuardChunkedDS", function(length)
        ...
        if (not NS_DS_NAME or not NS_DS_CURRENT or not NS_DS_TOTAL or not NS_DS_LENGTH or not NS_DS_DATA) then return end

        if (not chunks[NS_DS_NAME]) then
            chunks[NS_DS_NAME] = {}
        end
