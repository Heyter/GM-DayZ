...
function SWEP:DoPunchAnimation()
	...
	if (IsValid(viewModel)) then
		viewModel:SetPlaybackRate(self.animSpeed)
...
function SWEP:PrimaryAttack()
	...
	timer.Simple(0.055, function()
		if (IsValid(self) and IsValid(self:GetOwner())) then
			local damage = math.random(8, self.Primary.Damage)
...
function SWEP:SecondaryAttack()
	...
	if (SERVER and IsValid(entity)) then
		...
		elseif (entity:IsPlayer() and ix.config.Get("allowPush", true)) then
			if (!self:GetOwner():IsAdmin() and entity:GetLocalVar("SH_SZ.Safe", SH_SZ.OUTSIDE) == SH_SZ.PROTECTED) then
				return
			end
