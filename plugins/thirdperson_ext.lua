...
local view, traceData, traceData2, aimOrigin, curAng, owner
...
function PLUGIN:CalcView(client, origin, angles, fov)
	if (client:CanOverrideView() and LocalPlayer():GetViewEntity() == LocalPlayer()) then
		...
		traceData = {}
			...
			traceData.endpos = traceData.start + curAng:Right() * ix.option.Get("thirdpersonHorizontal", 0) + 
				curAng:Up() * ix.option.Get("thirdpersonVertical", 10) + 
				curAng:Forward() * -ix.option.Get("thirdpersonDistance", 50)
